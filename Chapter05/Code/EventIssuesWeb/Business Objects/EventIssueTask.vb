...
Public Class EventIssueTask
  ...
  Public Shared Function Add(ByRef EventIssueTaskObj As EventIssueTask) As Boolean
    ...
    Try
      Dim SQL As String = "INSERT INTO [Task](task_due_date, task_order, " & _
      "tracking_id, task, assignee_email, comments, complete, sent) " & _
      "VALUES (" & quoteDateForSQL(EventIssueTaskObj.TaskDueDate) & "," & _
      quoteForSQL(EventIssueTaskObj.TaskOrder) & "," & _
      quoteForSQL(EventIssueTaskObj.TrackingId) & "," & _
      quoteForSQL(EventIssueTaskObj.Task) & "," & _
      quoteForSQL(EventIssueTaskObj.AssigneeEmail) & "," & _
      quoteForSQL(EventIssueTaskObj.Comments) & "," & _
      quoteBooleanForSQL(EventIssueTaskObj.Complete) & ", false);"
  ...
  Public Shared Function Update(ByRef EventIssueTaskObj As EventIssueTask) As Boolean
    ...
    Try
      Dim SQL As String = "UPDATE [Task] SET [comments]=" & _
        quoteForSQL(EventIssueTaskObj.Comments) & ", [complete]=" & _
        quoteBooleanForSQL(EventIssueTaskObj.Complete) & " WHERE [task_id]=" & _
        EventIssueTaskObj.TaskId
      Dim dbConn As OleDbConnection = GetConnection()
      Dim dbCmd As New OleDbCommand(SQL, dbConn)
  ...
  Public Shared Function GetTasks(ByVal TrackingId As Long) As Collection
    ...
    Try
      ...
      Dim SQL As String = "SELECT * FROM [task] WHERE " & _
  ...
  Private Shared Function GetDataFromReader(ByRef dbReader _
    As OleDbDataReader) As EventIssueTask
